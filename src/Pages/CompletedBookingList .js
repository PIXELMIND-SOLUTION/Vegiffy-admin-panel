...
const CompletedBookingList = () => {
  ...
  const generateInvoicePDF = (booking) => {
    const doc = new jsPDF();
    doc.setFontSize(12);
    doc.text("Completed Booking Invoice", 14, 20);
    ...
    doc.save(`Completed_Invoice_${booking.id}.pdf`);
  };

  return (
    <div className="p-4 bg-white shadow-md rounded-md w-full max-w-full overflow-x-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-md font-semibold text-gray-700">Completed Orders</h2>
        <button
