...
import { FiEdit, FiTrash2, FiX, FiDownload, FiFilter, FiEye, FiUser, FiCheckCircle, FiXCircle, FiRefreshCw, FiSearch, FiMessageSquare, FiInfo } from 'react-icons/fi';
...
const VendorWithdrawalList = () => {
  ...
  const fetchWithdrawals = useCallback(async (showToast = false) => {
    try {
      setLoading(true);
      setError('');
  ...
  // Validate message based on status
  const isMessageRequired = useCallback(() => {
    return newStatus === 'approved' || newStatus === 'rejected';
  }, [newStatus]);

  const getMessagePlaceholder = useCallback(() => {
    if (newStatus === 'approved') {
      return "Enter approval message (e.g., Withdrawal approved and will be processed within 24 hours)...";
    } else if (newStatus === 'rejected') {
      return "Enter rejection reason (e.g., Insufficient balance, Invalid bank details, etc.)...";
    }
    return "Optional: Add any remarks or notes...";
  }, [newStatus]);
  ...
  // Handle status update with subAdminId and message
  const handleStatusUpdate = useCallback(async () => {
    ...
    // Validate message for approval or rejection
    if (isMessageRequired() && !statusMessage.trim()) {
      toast.error(`Please provide a ${newStatus === 'approved' ? 'approval message' : 'rejection reason'}`, {
        position: "top-right",
        autoClose: 4000,
      });
      return;
    ...
    try {
      ...
      const requestData = {
        ...
        note: `[${userInfo.role === "subadmin" ? "Sub-Admin" : "Admin"}: ${userInfo.name}] ${newStatus === 'approved' ? 'Approved' : 'Rejected'}: ${statusMessage}`
      };

      if (subAdminId) {
        requestData.subAdminId = subAdminId;
        requestData.processedBy = userInfo.name;
        requestData.processedByRole = "subadmin";
      } else {
        requestData.processedBy = userInfo.name;
        requestData.processedByRole = "admin";
      }
      ...
      if (response.data.success) {
        ...
        toast.success(
          `Withdrawal request ${newStatus} successfully! ${statusMessage ? `Message: ${statusMessage}` : ''}`,
          {
            position: "top-right",
            autoClose: 5000,
          }
  ...
  }, [selectedWithdrawal, newStatus, statusMessage, getSubAdminId, getUserInfo, isMessageRequired]);

  // Handle delete with subAdminId
  const handleDelete = useCallback(async (withdrawalId) => {
  ...
  const exportToCSV = useCallback(() => {
    try {
      const headers = ['Vendor Name', 'Vendor Email', 'Amount', 'Bank Name', 'Account Number', 'IFSC Code', 'Account Holder', 'Status', 'Status Message', 'Processed By', 'Date Requested', 'Last Updated'];
      ...
      const csvData = filteredWithdrawals.map(withdrawal => [
        ...
        withdrawal.status,
        withdrawal.statusMessage || withdrawal.note || 'N/A',
  ...
  // Open status modal
  const openStatusModal = useCallback((withdrawal) => {
  ...
  // Open view modal
  const openViewModal = useCallback((withdrawal) => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              {isAnyFilterActive && (
                <button onClick={resetFilters} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                  Clear all filters
                </button>
              )}
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Vendor Details</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Amount</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bank Details</th>
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status & Timeline</th>
                    ...
                    <tr key={withdrawal._id} className="hover:bg-blue-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          ...
                          {(withdrawal.note || withdrawal.statusMessage) && (
                            <div className="text-xs text-blue-600 italic mt-1">
                              <FiMessageSquare className="inline mr-1" size={10} />
                              {withdrawal.statusMessage || withdrawal.note}
                            </div>
                          )}
                        </div>
                      </td>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="text-xs text-gray-500">
                            ...
                            {withdrawal.processedBy && (
                              <div className="text-purple-600">By: {withdrawal.processedBy}</div>
                            )}
                          </div>
                        </div>
                      </td>
                ...
                {(newStatus === 'approved' || newStatus === 'rejected') && (
                  <div className="animate-fade-in">
                    ...
                    <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                      <FiInfo size={12} />
                      {newStatus === 'approved'
                        ? 'This message will be sent to the vendor as confirmation'
                        : 'This reason will help the vendor understand why their request was rejected'}
                    </p>

                    {/* Quick suggestion chips for rejection reasons */}
                    {newStatus === 'rejected' && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-600 mb-1">Common rejection reasons:</p>
                        <div className="flex flex-wrap gap-1">
                          {['Insufficient balance', 'Invalid bank details', 'Account verification failed', 'Duplicate request', 'Maximum limit exceeded'].map(reason => (
                            <button
                              key={reason}
                              type="button"
                              onClick={() => setStatusMessage(reason)}
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <StatCard title="Amount" value={`₹${selectedViewWithdrawal.amount}`} icon="💰" color="blue" />
                  <StatCard title="Status" value={formatStatus(selectedViewWithdrawal.status)} icon="📊"
                    color={selectedViewWithdrawal.status === 'approved' ? 'green' : selectedViewWithdrawal.status === 'rejected' ? 'red' : 'yellow'} />
                  <StatCard title="Vendor Wallet" value={`₹${selectedViewWithdrawal.restaurantId?.walletBalance?.toLocaleString() || '0'}`} icon="💳" color="purple" />
                  <StatCard title="Request ID" value={selectedViewWithdrawal._id.slice(-8)} icon="🆔" color="indigo" />
                </div>
                ...
                {(selectedViewWithdrawal.statusMessage || selectedViewWithdrawal.note) && (
                  <div className={`rounded-lg p-4 ${selectedViewWithdrawal.status === 'approved'
                      ? 'bg-green-50 border border-green-200'
                      : selectedViewWithdrawal.status === 'rejected'
                        ? 'bg-red-50 border border-red-200'
                        : 'bg-blue-50 border border-blue-200'
                    }`}>
                    <h4 className="font-medium text-gray-800 mb-2 flex items-center gap-2">
                      <FiMessageSquare />
                      {selectedViewWithdrawal.status === 'approved' ? 'Approval Message' :
                        selectedViewWithdrawal.status === 'rejected' ? 'Rejection Reason' : 'Remarks'}
                    </h4>
                    <p className="text-gray-700">{selectedViewWithdrawal.statusMessage || selectedViewWithdrawal.note}</p>
                    {selectedViewWithdrawal.processedBy && (
                      <p className="text-xs text-gray-500 mt-2">
                        Processed by: {selectedViewWithdrawal.processedBy}
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="flex justify-end mt-6 pt-6 border-t">
                <button onClick={() => setViewModalOpen(false)} className="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">
                  Close
                </button>
              </div>
            </div>
